A project to share your files in between different devices connected withing same local network.

I have not put this sites available publicly due to security issues. 

For now I want this project to keep within local network


This project is built using nodes js, mysql and react

To run this project

- git clone project url with branch file-sharing
- install necessaries packages for both backend and frontend using yarn install
- run both backend and frontend using yarn start
  
